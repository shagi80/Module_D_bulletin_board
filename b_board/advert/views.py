...
from .models import Advert, CATEGORY_CHOICES, Comment
...
class MainPage(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'category_id' in self.kwargs:
            context['current_category_title'] = CATEGORY_CHOICES[self.kwargs['category_id']][1]
            context['current_category_key'] = CATEGORY_CHOICES[self.kwargs['category_id']][0]
        return (context)


# обработка AJAX запроса выдачи блоков объявлений
# размер блок определяется константой внутри фугкции
...
class UserPage(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if 'category_id' in self.kwargs:
            adverts = adverts.filter(category=CATEGORY_CHOICES[self.kwargs['category_id']][0])
        return adverts